...
Viewers and analysts can read records; only admins can create, update, or delete.

Routes:
    POST   /api/records       — Create a new record (Admin only)
    GET    /api/records       — List records with filtering (All authenticated)
    GET    /api/records/:id   — Get a specific record (All authenticated)
